...
interface PersonDepositFundsProps {
  personAddress?: `0x${string}`;
  isLoading?: boolean;
}
...
export const PersonDepositFunds = ({
  isLoading,
  personAddress,
}: PersonDepositFundsProps) => {
  return (
    <div className="flex flex-col gap-5">
      ...
      <Skeleton loading={isLoading} width="100%" height={40}>
        <span className="flex justify-between items-center px-2 py-1 bg-secondary border borged-neutral-5 rounded-lg">
          <span className="text-neutral-9">{personAddress}</span>
          <CopyIcon
            className="cursor-pointer"
            onClick={() => {
              if (!isLoading) copyToClipboard(personAddress as string);
            }}
          />
        </span>
      </Skeleton>
