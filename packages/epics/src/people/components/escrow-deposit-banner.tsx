...
export const EscrowDepositBanner = ({ deposit, onDeposited }: Props) => {
  ...
  const needsApprove = deposit.allowanceB < deposit.amountB;
  const insufficientBalance = deposit.balanceB < deposit.amountB;

  const handleClick = React.useCallback(async () => {
    try {
      const hash = await sendDeposit({
        escrowId: deposit.escrowId,
        token: deposit.tokenB,
        amount: deposit.amountB,
        currentAllowance: deposit.allowanceB,
      });
      if (hash) {
  ...
  const sendLabel = `${formatAmount(deposit.amountB, deposit.tokenBDecimals)} ${
      ...
      <div className="flex items-start gap-3 md:gap-5 w-full md:w-auto">
        ...
        <div className="flex flex-col gap-2 flex-1">
          ...
          <span className="text-2 text-foreground">
            An investment proposal you are party to was approved. Deposit{' '}
            <span className="font-bold">{sendLabel}</span> into escrow to
            receive <span className="font-bold">{receiveLabel}</span> in
            exchange.
          </span>
          {insufficientBalance ? (
