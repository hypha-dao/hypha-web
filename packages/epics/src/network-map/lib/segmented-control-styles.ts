 ...
 * Densify to h-8 so projection / view tracks match Land·Water·Grid chips.
 */

/** Track behind exclusive projection / view switches. */
export const segmentedListClass =
  'inline-flex h-8 w-fit max-w-full shrink-0 items-center gap-0.5 rounded-lg bg-neutral-3 p-0.5';

/** Exclusive TabsTrigger inside a switch track. */
export const segmentedTriggerClass = cn(
  'flex h-7 flex-initial items-center justify-center gap-1 rounded-md px-2.5 py-0 text-xs font-medium shadow-none',
...
/** Track for independent layer toggles (Land / Water / Grid). */
export const segmentedLayerGroupClass =
  'flex h-8 w-fit max-w-full min-w-0 items-center gap-0.5 rounded-lg bg-neutral-3 p-0.5';
...
export function segmentedButtonClass(active: boolean) {
  return cn(
    'inline-flex h-7 min-w-0 flex-initial items-center justify-center gap-1 rounded-md px-2.5 py-0 text-xs font-medium sm:gap-1.5 sm:px-3 sm:text-sm',
