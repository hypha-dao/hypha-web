...
export const ProposalDetail = ({
  ...
  const resubmitTemplateData = (() => {
    ...
    if (label === 'Change Delegate') {
      ...
      return {
        changeDelegateTargetAddress: targetSpaceAddress,
        changeDelegateMemberAddress: dd.member,
      };
    }
  ...
  const exchangeDetails =
    label === 'Exchange' ? parseExchangeDetailsFromDescription(content) : null;
  const hasProposalDetails = Boolean(
    label === 'Investment' ||
      exchangeDetails ||
      proposalDetails?.votingMethods?.length ||
      proposalDetails?.entryMethods?.length ||
      proposalDetails?.tokenRequirements?.length ||
      (label !== 'Investment' &&
        label !== 'Exchange' &&
        proposalDetails?.tokens?.length) ||
      (proposalDetails?.transfers?.length &&
        label !== 'Investment' &&
        label !== 'Exchange') ||
      proposalDetails?.mintings?.length ||
      proposalDetails?.burnings?.length ||
        ...
        <details className="proposal-details-disclosure group rounded-xl border border-border/70 bg-background-2">
          ...
          <div className="flex flex-col gap-5 border-t border-border/60 px-4 py-4">
            ...
            {exchangeDetails ? (
              <ProposalExchangeStakesAndTokensData
                spaceSlug={spaceSlug}
                sellerAddress={exchangeDetails.sellerAddress}
                buyerAddress={exchangeDetails.buyerAddress}
                sellerLeg={exchangeDetails.sellerLeg}
                buyerLeg={exchangeDetails.buyerLeg}
                dbTokens={dbTokens}
              />
            ) : null}
            ...
            {proposalDetails?.delegatesData?.member ? (
              <ProposalDelegatesData
                ...
                variant={
                  label === 'Change Delegate' ? 'changeDelegate' : 'join'
                }
              />
            ) : null}
            {proposalDetails?.membershipExitData?.member ? (
            ...
            hasUpdateTokenDataToDisplay(proposalDetails.updateTokenData) ? (
              <ProposalUpdateToken
                documentId={documentId}
                address={
                  proposalDetails.updateTokenData.address as `0x${string}`
                }
                ...
                decayPercentage={
                  proposalDetails.updateTokenData.decayPercentage
                }
