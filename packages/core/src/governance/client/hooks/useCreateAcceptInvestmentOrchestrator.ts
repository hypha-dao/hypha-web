...
import { useAcceptInvestmentMutationsWeb3Rpc } from './useAcceptInvestmentMutations.web3.rpc';
import { useAgreementFileUploads } from './useAgreementFileUploads';
import {
  appendHyphaInvestmentFormMarker,
  type HyphaInvestmentFormPayloadV1,
} from '../escrow';

type CreateAcceptInvestmentArg = z.infer<typeof schemaAcceptInvestment> & {
  web3SpaceId?: number;
};
...
export const useCreateAcceptInvestmentOrchestrator = ({
  authToken,
  config,
}: {
  ...
  const web3 = useAcceptInvestmentMutationsWeb3Rpc({
  ...
  const { trigger: createAcceptInvestment } = useSWRMutation(
    ...
    async (_: string, { arg }: { arg: CreateAcceptInvestmentArg }) => {
      ...
      const markerPayload: HyphaInvestmentFormPayloadV1 = {
        version: 1,
  ...
  const { data: updatedWeb2Agreement } = useSWR(
    web2.createdAgreement?.slug &&
      ...
      ? [
          web2.createdAgreement.slug,
          web3.createdAcceptInvestment?.proposalId,
          'linkingWeb2AndWeb3AcceptInvestment',
        ]
      : null,
  ...
  const errors = useMemo(
    () =>
      [
        ...
        web3.errorCreateAcceptInvestment,
        web3.errorWaitAcceptInvestmentFromTransaction,
      ].filter(Boolean),
    [web2, web3],
  );
  ...
  const reset = useCallback(() => {
    ...
    web3.resetCreateAcceptInvestmentMutation();
  }, [resetTasks, web2, web3]);

  const isPending =
    (progress > 0 && progress < 100) ||
    Object.values(taskState).some((t) => t.status === TaskStatus.IS_PENDING);

  return {
    reset,
    createAcceptInvestment: createAcceptInvestment,
    agreement: {
      ...web2.createdAgreement,
      ...web3.createdAcceptInvestment,
