...
import { useRedeemTokensMutationsWeb3Rpc } from './useRedeemTokensMutations.web3.rpc';
...
export const useCreateRedeemTokensOrchestrator = ({
  authToken,
  config,
}: {
  ...
  const web3 = useRedeemTokensMutationsWeb3Rpc({
  ...
  const { trigger: createRedeemTokens } = useSWRMutation(
    'createRedeemTokensOrchestration',
    async (_: string, { arg }: { arg: CreateRedeemTokensArg }) => {
  ...
  const { data: updatedWeb2Agreement } = useSWR(
    web2.createdAgreement?.slug &&
      ...
      ? [
          web2.createdAgreement.slug,
          web3.createdRedeemTokens?.proposalId,
          'linkingWeb2AndWeb3',
        ]
      : null,
  ...
  const errors = useMemo(
    () =>
      [
        ...
        web3.errorCreateRedeemTokens,
        web3.errorWaitRedeemTokensFromTransaction,
      ].filter(Boolean),
    [web2, web3],
  );
  ...
  const reset = useCallback(() => {
    ...
    web3.resetCreateRedeemTokensMutation();
