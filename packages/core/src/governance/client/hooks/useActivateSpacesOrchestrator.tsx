...
import { useActivateSpacesMutationsWeb3Rpc } from './useActivateSpacesMutations.web3.rsc';
...
const taskActionDescriptions: Record<TaskName, string> = {
  ...
  CREATE_WEB3_ACTIVATION: 'Processing space activation...',
...
const initialTaskState: TaskState = {
  ...
  CREATE_WEB3_ACTIVATION: { status: TaskStatus.IDLE },
...
type ActivateSpacesArg = z.infer<typeof schemaActivateSpaces> & {
  spaceId: number;
  web3SpaceId: number;
};

export const useActivateSpacesOrchestrator = ({
  authToken,
  config,
}: {
  ...
  const web3 = useActivateSpacesMutationsWeb3Rpc({
      ...
      try {
        if (config) {
          ...
          await web3.activateInSpaces({
            spaceIds,
            amounts,
            paymentToken: arg.paymentToken,
            spaceId: arg.web3SpaceId,
          });
          completeTask('CREATE_WEB3_ACTIVATION');
        }
  ...
  const { data: updatedWeb2Agreement } = useSWR(
    web2.createdAgreement?.slug &&
      ...
      (!config ||
        taskState.CREATE_WEB3_ACTIVATION.status === TaskStatus.IS_DONE)
      ? [
          web2.createdAgreement.slug,
          web3.activationData?.proposalId,
          'linkingWeb2AndWeb3',
        ]
      : null,
  ...
  const errors = useMemo(
    () =>
      [
        ...
        web3.activateError,
        web3.errorWaitActivationFromTx,
      ].filter(Boolean),
    [web2, web3],
  );
