...
import { useSpaceToSpaceMembershipWeb3Rpc } from './useSpaceToSpaceMembershipMutations.web3.rsc';
...
type SpaceToSpaceMembershipArg = z.infer<
  typeof schemaSpaceToSpaceMembership
> & {
  member: string;
  space: string;
  web3SpaceId: number;
...
export const useSpaceToSpaceMembershipOrchestrator = ({
  authToken,
  config,
  spaces,
}: {
  ...
  const web3 = useSpaceToSpaceMembershipWeb3Rpc({
  ...
  const { trigger: spaceToSpaceAction } = useSWRMutation(
    'activateSpacesOrchestration',
    async (_: string, { arg }: { arg: SpaceToSpaceMembershipArg }) => {
      ...
      try {
        if (config) {
          ...
          await web3.spaceToSpaceMembership({
  ...
  const { data: updatedWeb2Agreement } = useSWR(
    web2.createdAgreement?.slug &&
      ...
      ? [
          web2.createdAgreement.slug,
          web3.membershipData?.proposalId,
          'linkingWeb2AndWeb3',
        ]
      : null,
  ...
  const errors = useMemo(
    () =>
      [
        ...
        web3.membershipError,
        web3.errorWaitMembershipFromTx,
      ].filter(Boolean),
    [web2, web3],
  );
  ...
  const reset = useCallback(() => {
    ...
    web3.resetSpaceToSpaceMembershipMutation();
