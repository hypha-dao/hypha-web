...
type MembershipExitArg = z.infer<typeof schemaMembershipExit> & {
  member: string;
  web3SpaceId: number;
};
...
export const useMembershipExitOrchestrator = ({
    ...
    async (_: string, { arg }: { arg: MembershipExitArg }) => {
      ...
      try {
        if (config) {
          ...
          await web3.membershipExit({
            memberAddress: arg.member as `0x${string}`,
  ...
  const reset = useCallback(() => {
    ...
    web3.resetMembershipExitMutation();
