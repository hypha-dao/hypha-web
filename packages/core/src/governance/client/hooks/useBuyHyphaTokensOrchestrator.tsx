...
import { useBuyHyphaTokensMutationsWeb3Rpc } from './useBuyHyphaTokensMutations.web3.rsc';
...
const taskActionDescriptions: Record<TaskName, string> = {
  ...
  CREATE_WEB3_INVESTMENT: 'Processing token investment...',
...
const initialTaskState: TaskState = {
  ...
  CREATE_WEB3_INVESTMENT: { status: TaskStatus.IDLE },
...
type BuyHyphaTokensArg = z.infer<typeof schemaBuyHyphaTokens> & {
  web3SpaceId?: number | null;
};

export const useBuyHyphaTokensOrchestrator = ({
  authToken,
  config,
}: {
  ...
  const web3 = useBuyHyphaTokensMutationsWeb3Rpc({
  ...
  const { trigger: buyHyphaTokens } = useSWRMutation(
    'buyHyphaTokensOrchestration',
    async (_: string, { arg }: { arg: BuyHyphaTokensArg }) => {
      ...
      try {
        if (config) {
          startTask('CREATE_WEB3_INVESTMENT');
          await web3.investInHypha({
            usdcAmount: arg.payout.amount,
            spaceId: arg.web3SpaceId as number,
          });
          completeTask('CREATE_WEB3_INVESTMENT');
        }
  ...
  const { data: updatedWeb2Agreement } = useSWR(
    web2.createdAgreement?.slug &&
      ...
      (!config ||
        taskState.CREATE_WEB3_INVESTMENT.status === TaskStatus.IS_DONE)
      ? [
          web2.createdAgreement.slug,
          web3.investmentData?.proposalId,
          'linkingWeb2AndWeb3',
        ]
      : null,
  ...
  const errors = useMemo(
    () =>
      [
        ...
        web3.investError,
        web3.errorWaitInvestmentFromTx,
      ].filter(Boolean),
    [web2, web3],
  );
