...
import { useTokenBackingVaultMutationsWeb3Rsc } from './useTokenBackingVaultMutations.web3.rsc';

type CreateTokenBackingVaultArg = z.infer<typeof schemaCreateAgreement> & {
  tokenBackingVault: {
    spaceToken: string;
    activateVault: boolean;
    enableRedemption: boolean;
    addCollaterals?: Array<{ token: string; amount: string }>;
...
export const useCreateTokenBackingVaultOrchestrator = ({
  authToken,
  config,
}: {
  ...
  const web3 = useTokenBackingVaultMutationsWeb3Rsc({
  ...
  const { trigger: createTokenBackingVault } = useSWRMutation(
    'createTokenBackingVaultOrchestration',
    async (_: string, { arg }: { arg: CreateTokenBackingVaultArg }) => {
      ...
      try {
        if (config && typeof web3SpaceId === 'number') {
          ...
          await web3.createTokenBackingVaultProposal({
            spaceId: web3SpaceId,
            spaceToken: arg.tokenBackingVault.spaceToken as `0x${string}`,
            activateVault: arg.tokenBackingVault.activateVault,
            enableRedemption: arg.tokenBackingVault.enableRedemption,
            addCollaterals: arg.tokenBackingVault.addCollaterals,
            removeCollaterals: arg.tokenBackingVault.removeCollaterals,
            referenceCurrency: arg.tokenBackingVault.referenceCurrency,
            tokenPrice: arg.tokenBackingVault.tokenPrice,
            minimumBackingPercent: arg.tokenBackingVault.minimumBackingPercent,
            maxRedemptionPercent: arg.tokenBackingVault.maxRedemptionPercent,
            maxRedemptionPeriodDays:
              arg.tokenBackingVault.maxRedemptionPeriodDays,
            redemptionStartDate: arg.tokenBackingVault.redemptionStartDate,
            enableAdvancedRedemptionControls:
              arg.tokenBackingVault.enableAdvancedRedemptionControls,
  ...
  const { data: updatedWeb2Agreement } = useSWR(
    web2.createdAgreement?.slug &&
      ...
      ? [
          web2.createdAgreement.slug,
          web3.createdTokenBackingVault?.proposalId,
          'linkingWeb2AndWeb3',
        ]
      : null,
  ...
  const errors = useMemo(
    () =>
      [
        ...
        web3.errorCreateTokenBackingVault,
        web3.errorWaitTokenBackingVaultFromTransaction,
      ].filter(Boolean),
    [web2, web3],
  );
  ...
  const reset = useCallback(() => {
    ...
    web3.resetCreateTokenBackingVaultMutation();
