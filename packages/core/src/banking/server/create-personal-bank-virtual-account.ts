...
import type {
  CreatePersonalBankVirtualAccountInput,
  CreatePersonalBankVirtualAccountResult,
} from '../types';
...
export type CreatePersonalBankVirtualAccountOptions = {
  kycProvider?: BankKycProvider;
};

const PERSONAL_NOT_APPROVED_MESSAGE =
  'Complete identity verification before opening bank accounts';

export async function createPersonalBankVirtualAccount(
  input: CreatePersonalBankVirtualAccountInput,
  { db }: { db: DatabaseInstance },
  options?: CreatePersonalBankVirtualAccountOptions,
): Promise<CreatePersonalBankVirtualAccountResult> {
  ...
  return createBankVirtualAccountForCustomer(
    ...
    walletAddress,
    { ...options, notApprovedMessage: PERSONAL_NOT_APPROVED_MESSAGE },
  );
}
