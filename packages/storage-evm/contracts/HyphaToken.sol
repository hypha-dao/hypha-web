...
interface ISpacesContract {
    function updateSpacePayment(address user, uint256 spaceId, uint256 durationInDays) external;
}

contract HyphaToken is ERC20, ERC20Mintable, Ownable {
    ...
    uint256 public distributionMultiplier = 10; // Can be updated by governance
    ...
    // Events
    event SpacePaymentProcessed(address indexed user, uint256 spaceId, uint256 durationInDays, uint256 usdcAmount, uint256 hyphaMinted);
    event HyphaInvestment(address indexed investor, uint256 usdcAmount, uint256 hyphaPurchased);
    ...
    constructor(address _usdc, address _spacesContract) ERC20("HYPHA", "HYPHA") {
        usdc = IERC20(_usdc);
     ...
     */
    function payForSpaces(uint256 usdcAmount, uint256 spaceId, uint256 durationInDays) external {
        ...
        require(totalMinted.add(hyphaMinted) <= MAX_SUPPLY, "Exceeds max token supply");
        ...
        emit SpacePaymentProcessed(msg.sender, spaceId, durationInDays, usdcAmount, hyphaMinted);
    }
    ...
    function investInHypha(uint256 usdcAmount) external {
        ...
        // Ensure we don't exceed max supply
        require(totalMinted.add(hyphaPurchased) <= MAX_SUPPLY, "Exceeds max token supply");
    ...
    function pendingRewards(address user) public view returns (uint256) {
        ...
        if (block.timestamp > lastUpdateTime && totalSupply() > 0 && pendingDistribution > 0) {
            uint256 timeElapsed = block.timestamp.sub(lastUpdateTime);
            uint256 DISTRIBUTION_PERIOD = 1 days;
            uint256 emissionRate = pendingDistribution.div(DISTRIBUTION_PERIOD);
            uint256 toDistribute = timeElapsed.mul(emissionRate);
        ...
        uint256 newRewards = balance.mul(currentAccumulator.sub(userRewardDebt[user]));
    ...
    function claimRewards() external {
        ...
        if (reward > 0) {
            // Ensure we don't exceed max supply
            require(totalMinted.add(reward) <= MAX_SUPPLY, "Exceeds max token supply");
     ...
     * Since transfers are disabled, we're only keeping the base functionality
     */
