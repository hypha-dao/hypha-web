...
const PROXY_ADDRESS = '0xc87546357EeFF8653cF058Be2BA850813e39cda0';
...
async function main(): Promise<void> {
  ...
  console.log(
    ...
    VotingPowerDelegation.bytecode.length,
  );

  let upgradedContract;

  try {
    console.log('Upgrading VotingPowerDelegation...');
    ...
    if (error instanceof Error && error.message.includes('is not registered')) {
      ...
      try {
        ...
        await upgrades.forceImport(PROXY_ADDRESS, VotingPowerDelegation, {
          kind: 'uups',
        });
  ...
  console.log(
    'VotingPowerDelegation proxy address (unchanged):',
