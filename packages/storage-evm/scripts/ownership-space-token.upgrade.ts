...
async function main(): Promise<void> {
  ...
  console.log('Contract bytecode length:', OwnershipSpaceToken.bytecode.length);

  let upgradedContract;

  try {
    console.log('Upgrading OwnershipSpaceToken...');
    ...
    if (error instanceof Error && error.message.includes('is not registered')) {
      ...
      try {
        ...
        await upgrades.forceImport(PROXY_ADDRESS, OwnershipSpaceToken);
  ...
  console.log(
    'OwnershipSpaceToken proxy address (unchanged):',
