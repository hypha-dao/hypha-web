...
async function testProposalRejectionAndTracking(): Promise<void> {
  console.log('Starting proposal rejection and tracking test...');
  ...
  // Contract addresses debugging info
  ...
  const spaceParams: SpaceCreationParams = {
    unity: 51, // 51% unity - single address can trigger acceptance/rejection
    quorum: 51, // 51% quorum - single address can pass quorum
  ...
  try {
    ...
    // Check space proposals before creating the new proposal
    console.log('\nChecking space proposals before proposal creation...');
    try {
      const [acceptedBefore, rejectedBefore] =
        await daoProposals.getSpaceProposals(spaceId);
      console.log(`Accepted proposals before: [${acceptedBefore.join(', ')}]`);
      console.log(`Rejected proposals before: [${rejectedBefore.join(', ')}]`);
    } catch (error) {
      console.log('Could not fetch space proposals before creation:', error);
    }
    ...
    try {
      ...
      // Step 4: Vote NO on the proposal
      console.log('\nVoting NO on the proposal...');
      const voteTx = await daoProposals.vote(proposalId, false); // Vote NO (false)
      console.log(`Vote transaction hash: ${voteTx.hash}`);

      await voteTx.wait();
      console.log('NO vote confirmed');
      ...
      // Step 6: Check if the proposal appears in getSpaceProposals
      console.log('\nChecking space proposals after voting...');
      try {
        const [acceptedAfter, rejectedAfter] =
          await daoProposals.getSpaceProposals(spaceId);
        console.log(`Accepted proposals after: [${acceptedAfter.join(', ')}]`);
        console.log(`Rejected proposals after: [${rejectedAfter.join(', ')}]`);

        const isInRejected = rejectedAfter.some(
          (id) => Number(id) === proposalId,
        );
        const isInAccepted = acceptedAfter.some(
          (id) => Number(id) === proposalId,
        );

        console.log(`\nProposal ${proposalId} tracking results:`);
        console.log(`- Found in rejected list: ${isInRejected}`);
        console.log(`- Found in accepted list: ${isInAccepted}`);

        if (proposalData.expired && isInRejected) {
          console.log(
            '✅ SUCCESS! The proposal was properly rejected and appears in the rejected proposals list.',
          );
        } else if (proposalData.expired && !isInRejected) {
          console.log(
            '⚠️  The proposal was rejected but does NOT appear in the rejected proposals list.',
          );
        } else if (!proposalData.expired) {
          console.log(
            'ℹ️  The proposal has not been rejected yet (may need more votes or time to expire).',
          );
        }
      } catch (error) {
        console.error('Error checking space proposals:', error);
      }

      if (proposalData.executed) {
        console.log('Unexpected: The proposal was executed despite voting NO.');
      } else if (proposalData.expired) {
        console.log('Expected: The proposal was rejected/expired.');
      } else {
        console.log('The proposal is still active.');
    ...
    console.error('Error in proposal rejection test process:', outerError);
  }
}

// Run the test
testProposalRejectionAndTracking().catch(console.error);
