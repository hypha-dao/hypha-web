...
async function main(): Promise<void> {
  ...
  if (!ethers.isAddress(usdcTokenAddress)) {
    throw new Error(`Invalid USDC address: ${usdcTokenAddress}`);
  }
  ...
  console.log(`🚀 HYPHA TOKENOMICS VERIFICATION - 0.5 USDC PAYMENT`);
  console.log(`Using wallet address: ${wallet.address}`);
  console.log(`HyphaToken Contract: ${hyphaTokenAddress}`);
  console.log(`USDC Contract: ${usdcTokenAddress}\n`);
  ...
  const usdcToken = new ethers.Contract(
    ...
    usdcTokenAbi,
    wallet,
  ) as ethers.Contract & UsdcTokenInterface;
  ...
  try {
    ...
    const initialUserUsdcBalance = await usdcToken.balanceOf(wallet.address);
    ...
    console.log(
      `User USDC Balance: ${ethers.formatUnits(
    ...
    if (initialUserUsdcBalance < usdcAmount) {
      throw new Error(
    ...
    if (currentAllowance < usdcAmount) {
      console.log(
        `Current allowance is ${ethers.formatUnits(
          currentAllowance,
          6,
        )} USDC. Approving ${ethers.formatUnits(usdcAmount, 6)} USDC...`,
      );
      const approveTx = await usdcToken.approve(hyphaTokenAddress, usdcAmount);
      console.log(`Approval transaction sent: ${approveTx.hash}`);
      await approveTx.wait();
      console.log('✅ USDC approved successfully!');
    } else {
      console.log(
        `✅ Sufficient USDC allowance already exists: ${ethers.formatUnits(
          currentAllowance,
          6,
        )} USDC`,
      );
    ...
    // STEP 3: Calculate expected changes
    console.log('\n🧮 STEP 3: EXPECTED TOKENOMICS');
    console.log('='.repeat(60));

    const hyphaPriceUsd = await hyphaToken.HYPHA_PRICE_USD();
    const hyphaEquivalent =
      (usdcAmount * 4n * 10n ** 12n) /
      (hyphaPriceUsd === 0n ? 1n : hyphaPriceUsd);
    const expectedRewardGeneration =
      hyphaEquivalent * (distributionMultiplier + 1n);

    const expectedNewPendingDistribution =
      initialPendingDistribution + expectedRewardGeneration;

    console.log(`Payment Amount: ${ethers.formatUnits(usdcAmount, 6)} USDC`);
    console.log(
      `Expected HYPHA Equivalent: ${ethers.formatEther(hyphaEquivalent)} HYPHA`,
    );
    console.log(
    ...
    // Get USDC_PER_DAY for duration calculation
    try {
      const usdcPerDay = await hyphaToken.USDC_PER_DAY();
      const usdcAmountNumber = Number(ethers.formatUnits(usdcAmount, 6));
      const usdcPerDayNumber = Number(ethers.formatUnits(usdcPerDay, 6));
      const durationInDays = usdcAmountNumber / usdcPerDayNumber;
      console.log(
        `USDC per day rate: ${ethers.formatUnits(usdcPerDay, 6)} USDC`,
      );
      console.log(`Duration: ~${Math.round(durationInDays * 100) / 100} days`);
    } catch (error) {
      console.log('Could not fetch USDC_PER_DAY rate');
    }
    ...
    console.log(
      `Paying ${ethers.formatUnits(
        usdcAmount,
        6,
      )} USDC for space ${spaceId}...`,
    );

    const tx = await hyphaToken.payForSpaces([spaceId], [usdcAmount]);
    ...
    // STEP 5: Verify immediate changes
    console.log('\n📈 STEP 5: IMMEDIATE POST-PAYMENT VERIFICATION');
    console.log('='.repeat(60));

    const postUserUsdcBalance = await usdcToken.balanceOf(wallet.address);
    const postIexUsdcBalance = await usdcToken.balanceOf(iexAddress);
    const postPendingDistribution = await hyphaToken.pendingDistribution();

    const userUsdcBalanceChange = postUserUsdcBalance - initialUserUsdcBalance;
    const pendingDistributionChange =
      postPendingDistribution - initialPendingDistribution;

    console.log(
      `User USDC Balance Change: ${ethers.formatUnits(
        userUsdcBalanceChange,
        6,
      )} USDC`,
    );
    console.log(
    ...
    console.log(
      `IEX Address USDC Balance: ${ethers.formatUnits(
        postIexUsdcBalance,
        6,
      )} USDC`,
    );
    ...
    if (userUsdcBalanceChange === -usdcAmount) {
      console.log('✅ User USDC balance decreased correctly');
    } else {
      console.log(
        `❌ User USDC balance change mismatch. Expected: -${ethers.formatUnits(
          usdcAmount,
          6,
        )}, Actual: ${ethers.formatUnits(userUsdcBalanceChange, 6)}`,
      );
    }
    ...
    // STEP 6: Wait and check reward distribution
    ...
    // STEP 7: Comprehensive rewards verification
    console.log('\n🎯 STEP 7: COMPREHENSIVE REWARDS VERIFICATION');
    console.log('='.repeat(60));
    ...
    console.log('='.repeat(60));
    console.log(
      `✅ Successfully paid ${ethers.formatUnits(
        usdcAmount,
        6,
      )} USDC for space ${spaceId}`,
    );
    ...
    console.log('\n🔍 Key Metrics:');
    console.log(
      `- Payment: ${ethers.formatUnits(usdcAmount, 6)} USDC → IEX Address`,
    );
    console.log(
      `- Rewards Generated: ${ethers.formatEther(
        expectedRewardGeneration,
    ...
    if (error.message.includes('Insufficient USDC balance')) {
      console.log('\n💡 Tip: You need more USDC tokens to make this payment.');
