...
async function main(): Promise<void> {
  ...
  console.log('Contract bytecode length:', HyphaToken.bytecode.length);

  let upgradedContract;

  try {
    console.log('Upgrading HyphaToken...');

    // Try with options to get more detailed error info
    upgradedContract = await upgrades.upgradeProxy(PROXY_ADDRESS, HyphaToken, {
    ...
    if (error instanceof Error && error.message.includes('is not registered')) {
      ...
      try {
        ...
        await upgrades.forceImport(PROXY_ADDRESS, HyphaToken);
  ...
  console.log(
    'HyphaToken proxy address (unchanged):',
