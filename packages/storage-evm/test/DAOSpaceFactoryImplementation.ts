...
describe('DAOSpaceFactoryImplementation', function () {
  ...
  async function deployDAOSpaceFactoryFixture() {
    ...
    const MockProposalManager = await ethers.getContractFactory(
      'MockProposalManager',
    );
  ...
  describe('Deployment', function () {
    it('Should set the right owner', async function () {
      const { daoSpaceFactory, owner } = await loadFixture(
        deployDAOSpaceFactoryFixture,
      );
      expect(await daoSpaceFactory.owner()).to.equal(owner.address);
    });

    it('Should initialize with zero spaces', async function () {
      const { daoSpaceFactory } = await loadFixture(
        deployDAOSpaceFactoryFixture,
      );
  ...
  describe('Space Creation', function () {
    it('Should create a new space with correct parameters', async function () {
      const { daoSpaceFactory, mockProposalManager, owner } = await loadFixture(
        deployDAOSpaceFactoryFixture,
      );
    ...
    it('Should fail if space name is empty', async function () {
      const { daoSpaceFactory, mockProposalManager } = await loadFixture(
        deployDAOSpaceFactoryFixture,
      );
      ...
      await expect(daoSpaceFactory.createSpace(spaceParams)).to.be.revertedWith(
  ...
  describe('Space Membership', function () {
    it('Should allow joining a space', async function () {
      const { daoSpaceFactory, mockProposalManager, owner, otherAccount } =
        await loadFixture(deployDAOSpaceFactoryFixture);
