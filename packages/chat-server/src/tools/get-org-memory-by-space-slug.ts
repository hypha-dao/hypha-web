...
export function createGetOrgMemoryBySpaceSlugTool(
  ...
  return {
    description:
      'Read-only: organisation memory for a Hypha space by slug — same member roster as get_people_by_space_slug plus org_memory_assets (proposal attachments and lead images from documents; Matrix chat m.file/m.image when NEXT_PUBLIC_MATRIX_HOMESERVER_URL and chat_room_id are set, and either HYPHA_MATRIX_BOT_AS_TOKEN is set **or** the signed-in user has a Matrix link from Human Chat — then used_session_matrix_token is true). Each successful result includes matrix_fetch: skipped_reason (missing_homeserver_url | missing_access_token | missing_chat_room_id), used_bot_access_token, used_session_matrix_token, session_matrix_token_unavailable (JWT ok but no Matrix session / expired token), homeserver_configured, access_token_configured (HYPHA_MATRIX_BOT_AS_TOKEN only — false does NOT mean Matrix is impossible if used_session_matrix_token is true), http_status, events_in_chunk, media_events_yielded, hypha_media_bundle_slots, error — use these for why Matrix rows are empty; do not infer failure only from access_token_configured or missing bot token; do not blame generic access permissions. Optional assets_page, assets_page_size, assets_search paginate/filter assets separately from the roster. Not a substitute for get_documents_by_space_slug for governance workflow (state, status, creator) on each document row.',
    inputSchema,
    execute: async (args) => {
