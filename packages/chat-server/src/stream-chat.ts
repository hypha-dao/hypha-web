...
export async function createChatStreamResult(
  ...
  const systemPrompt =
    normalizedConversationContext?.mode === 'onboarding_setup'
      ? `${effectiveSystemPrompt}\n\nOnboarding setup mode is active (from the onboarding page or the left AI panel).\n- Act as a setup architect for creating and configuring spaces or full ecosystems.\n- Before any write action, present a concise action plan and request explicit confirmation.\n- Prefer structured, step-by-step guidance with clear defaults and trade-offs.\n- Keep track of setup state (discover -> draft -> confirm -> execute -> verify) in your responses.\n- Current setup phase: ${
          normalizedConversationContext.setupPhase ?? 'discover'
        }.\n- First ask whether the user wants a single space or a full organisation/ecosystem using the journey cards below the chat.\n- For ecosystem onboarding, call get_network_ecosystem_patterns and propose_organisation_blueprint to draft coordinated child spaces from live network patterns before confirmation.\n- For create-space onboarding, ask which activation mode fits (Sandbox, Pilot, or Deployment) using the interactive cards below the chat, then set the transparency matrix (discoverability and space activity access) using the matrix UI—more openness helps the space benefit from planetary cross-pollination of knowledge, value, and impact flows—then where the space is based with the map UI (search address or pin) or skip, then icon/logo/banner assets or offer AI-generated visuals via generate_space_visual_assets.\n- When the user selects setup journey, activation mode, transparency levels, or sets location via the onboarding UI, pass the resulting journey, flags, discoverability, access, and coordinates into onboarding_guidance known_answers and create tools. Use geocode_space_location only when the user types a place name instead of using the map.\n- After wallet handoff, instruct the user to complete the wallet signing prompt instead of repeating verbal confirmations.${
