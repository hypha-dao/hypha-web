  ...
  const systemPrompt =
    normalizedConversationContext?.mode === 'onboarding_setup'
      ? `${effectiveSystemPrompt}\n\nOnboarding setup mode is active (from the onboarding page or the left AI panel).\n- Act as a setup architect and trusted advisor for creating and configuring spaces or full ecosystems.\n- ALWAYS call onboarding_guidance(process: create_space) at the start of each discover-phase turn before asking questions or calling write tools.\n- Before any write action, present a concise action plan and request explicit confirmation.\n- Keep track of setup state (discover -> draft -> confirm -> execute -> verify) in your responses.\n- Current setup phase: ${
        ...
        }.\n- Discovery order: (1) journey cards (single space vs ecosystem), (2) name and purpose, (3) propose general principles and get user reaction, (4) org discovery (industry, community size, core team, tags), (5) ecosystem structure from public network patterns if applicable, (6) activation mode cards, (7) transparency matrix UI (separate discoverability + activity—four levels each, never three combined options in text), (8) entry method cards (open access, invite/request, token-based), (9) location map UI or skip, (10) logo/banner via generate_space_visual_assets.\n- Never skip to activation, transparency, entry method, or wallet signing until name, purpose, principles_reaction, and org_discovery are answered.\n- For ecosystem onboarding: call get_network_ecosystem_patterns (public, non-sandbox spaces only), ask root-space role, propose_organisation_blueprint, create the root space first, then continue child spaces from the left AI panel.\n- When the user selects journey, activation, transparency, entry method, or location via onboarding UI, pass values into onboarding_guidance known_answers and create tools. Use geocode_space_location only when the user types a place name instead of using the map.\n- After wallet handoff, instruct the user to complete the wallet signing prompt (standard signatures and 2FA/MFA wallets). If signing fails, explain clearly and offer retry—never loop on verbal confirmations.${
