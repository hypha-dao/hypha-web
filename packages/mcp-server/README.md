...
For **Matrix-backed** rows in `org_memory_assets`, set **`HYPHA_MATRIX_ORG_MEMORY_ACCESS_TOKEN`** (a normal Matrix **access token** for a user joined to the space chat room — *not* a Privy JWT) and **`NEXT_PUBLIC_MATRIX_HOMESERVER_URL`**. The server calls `GET /_matrix/client/v3/rooms/{roomId}/messages` with `access_token` as a query parameter (Synapse-compatible). Ensure the space has **`chat_room_id`** set in the database. Tool output includes **`matrix_fetch`** (HTTP status, event counts, skip reason) when Matrix rows are empty. Without env vars, proposal-backed assets still appear; Matrix assets are omitted.
