...
const spaceCreatedSigningKey = process.env.WH_SPACE_CREATED_SIGN_KEY;

export const POST = spaceCreatedSigningKey
  ? Alchemy.newHandler(
      {
        signingKey: spaceCreatedSigningKey,
        abi: daoSpaceFactoryImplementationAbi,
        event: 'SpaceCreated',
      },
      async (events) => {
        const creatorsWithTotalOfSpaces = events.reduce(
          (creators, { args }) => {
        ...
        const dbRequestForCreators = Object.entries(
          creatorsWithTotalOfSpaces,
        ).map(async ([address, spaces]) => {
  ...
  : async () => {
      console.error('Missing key for space created webhook');
