...
async function selectVotingTemplate(
  ...
  try {
    ...
    if (
      !(await templateElement.isVisible({ timeout: 5000 }).catch(() => false))
    ) {
...
async function selectVotingPower(
  ...
  try {
    ...
    if (
      !(await optionElement.isVisible({ timeout: 5000 }).catch(() => false))
    ) {
...
async function fillMinimumVotingDuration(page: Page): Promise<void> {
  console.log(
    '⏱️ Filling Minimum Voting Duration (required when quorum < 20%)...',
  );
  ...
  if (
    await votingPeriodSection.isVisible({ timeout: 3000 }).catch(() => false)
  ) {
  ...
  const votingDurationLabel = page
    .locator('text="Minimum Voting Duration"')
    .first();
  if (
    await votingDurationLabel.isVisible({ timeout: 3000 }).catch(() => false)
  ) {
  ...
  for (const dropdownSelector of dropdownSelectors) {
    if (
      await dropdownSelector.isVisible({ timeout: 1000 }).catch(() => false)
    ) {
      ...
      if (
        await sixHoursOption.isVisible({ timeout: 2000 }).catch(() => false)
      ) {
      ...
      const anyDurationOption = page
        .locator('text=/\\d+\\s*(hour|day|week)/i')
        .first();
      if (
        await anyDurationOption.isVisible({ timeout: 1000 }).catch(() => false)
      ) {
  ...
  if (!durationSet) {
    ...
    const lastResort = page
      .locator('button:near(:text("Minimum Voting Duration"))')
      .first();
...
async function fillVotingTokenAllocation(page: Page): Promise<void> {
  ...
  const votingTokenSection = page
    .locator('text="Voting Token Allocation"')
    .first();
  if (
    await votingTokenSection.isVisible({ timeout: 3000 }).catch(() => false)
  ) {
  ...
  if (await tokenDropdown.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    if (!tokenSelected) {
      ...
      const tokenWithQA = page
        .locator('div:has-text("by QA")')
        .filter({
    ...
    if (!tokenSelected) {
      ...
      const popoverItems = page.locator(
        '[data-radix-popper-content-wrapper] >> div[class*="cursor"], [data-radix-popper-content-wrapper] >> div[class*="hover"]',
      );
    ...
    if (!tokenSelected) {
      console.log(
        '⚠️ Could not select a token - dropdown may need manual interaction',
      );
    }
  } else {
    ...
    const dropdownButton = page
      .locator('button:has-text("Select a token"), [role="combobox"]')
      .last();
...
test.describe('Change Voting Method on Production', () => {
  ...
  test('should change voting method with randomized settings in the QA TESTING space', async ({
    ...
    const needsMinDuration =
      templatesRequiringDuration.includes(selectedTemplate);
    ...
    if (needsMinDuration) {
      console.log(
        `║    ⏱️ Min Voting Duration: 6 hours (template requires it)      ║`,
      );
    }
    ...
    for (let i = 0; i < 8; i++) {
      const votingMethodVisible = await page
        .locator('text=Voting Method')
        .first()
        .isVisible()
        .catch(() => false);
    ...
    const votingMethodCard = page
      .locator('text=Select and configure the voting method')
      .first();

    if (
      await votingMethodCard.isVisible({ timeout: 5000 }).catch(() => false)
    ) {
    ...
    console.log(
      `📋 Step 1: Selecting voting template "${selectedTemplate}"...`,
    );
    ...
    if (needsMinDuration) {
      console.log(
        `⚠️ "${selectedTemplate}" template has quorum < 20% - filling Minimum Voting Duration...`,
      );
      await fillMinimumVotingDuration(page);
    } else {
      // Also check if Minimum Voting Duration field is visible (just in case)
      const minDurationLabel = page
        .locator('text="Minimum Voting Duration"')
        .first();
      if (
        await minDurationLabel.isVisible({ timeout: 2000 }).catch(() => false)
      ) {
    ...
    if (
      await votingPowerSection.isVisible({ timeout: 3000 }).catch(() => false)
    ) {
      await votingPowerSection.scrollIntoViewIfNeeded();
    } else {
      // Try scrolling within the panel
      for (let i = 0; i < 5; i++) {
        const votingPowerVisible = await page
          .locator('text=Voting Power')
          .first()
          .isVisible()
          .catch(() => false);
    ...
    console.log(
      `⚡ Step 4: Selecting voting power "${selectedVotingPower}"...`,
    );
    ...
    if (
      selectedVotingPower === '1 Voice 1 Vote' ||
      selectedVotingPower === '1 Token 1 Vote'
    ) {
    ...
    for (let i = 0; i < 5; i++) {
      const publishButton = page
        .locator(
          'button:has-text("Publish"), button:has-text("Save"), button:has-text("Apply"), button:has-text("Confirm"), button:has-text("Update")',
        )
        .last();
