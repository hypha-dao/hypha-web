...
/**
 * Production Test: Deploy Funds
 *
 * ⚠️  WARNING: This test creates REAL data on production!
 * ⚠️  The deploy funds proposal will appear on https://app.hypha.earth
 ...
 *
 * Run this test:
 *   npx nx e2e-production-create-deploy-funds web-e2e -- --headed
 */
...
function createTestImageBuffer(): Buffer {
  // Minimal valid PNG - a 100x100 green image (different color for deploy funds)
  ...
  for (let y = 0; y < 100; y++) {
    ...
    for (let x = 0; x < 100; x++) {
      // Green color for deploy funds image
...
test.describe('Deploy Funds on Production', () => {
  ...
  test('should create a deploy funds proposal with title and description', async ({
    page,
  }) => {
    // Generate unique deploy funds name with timestamp
    const timestamp = Date.now();
    const randomSuffix = Math.random().toString(36).substring(2, 6);
    const deployFundsTitle = `E2E Test Deploy Funds ${timestamp}${randomSuffix}`;
    const deployFundsDescription =
      'This deploy funds proposal was created by automated E2E tests. It can be safely deleted or rejected.';
    ...
    console.log(
      '║  🚀 CREATING DEPLOY FUNDS PROPOSAL ON PRODUCTION               ║',
    );
    console.log(`║  Title: ${deployFundsTitle.substring(0, 52).padEnd(52)}║`);
    console.log(
      '╚════════════════════════════════════════════════════════════════╝',
    );
    console.log('');
    ...
    // Scroll down in the create menu to find "Deploy funds"
    console.log('📜 Scrolling to find "Deploy funds"...');
    
    // Look for the deploy funds option (may need to scroll)
    const deployFundsOption = page.locator('text=Deploy funds').first();
    
    // Try scrolling the menu if not visible
    if (!(await deployFundsOption.isVisible({ timeout: 2000 }).catch(() => false))) {
      // Scroll down within the create menu
      const createMenu = page.locator('[role="menu"], [role="dialog"], [class*="menu"], [class*="dropdown"]').first();
      if (await createMenu.isVisible().catch(() => false)) {
        await createMenu.evaluate((el) => el.scrollBy(0, 300));
        await page.waitForTimeout(500);
      }
    }
    
    // Select "Deploy funds" from the options
    console.log('📋 Selecting "Deploy funds"...');
    await deployFundsOption.scrollIntoViewIfNeeded();
    await expect(deployFundsOption).toBeVisible({ timeout: 5000 });
    await deployFundsOption.click();
    
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);

    console.log('📝 Filling deploy funds form...');
    ...
    const leadImagePath = path.join(
      process.cwd(),
      `test-results-production/test-deploy-funds-lead-image-${timestamp}.png`,
    );
    const attachmentPath = path.join(
      process.cwd(),
      `test-results-production/test-deploy-funds-attachment-${timestamp}.png`,
    );
    ...
    // Fill in the deploy funds title first
    console.log('📝 Filling title...');
    await titleInput.fill(deployFundsTitle);
    ...
    await page.waitForTimeout(500);
    await page.keyboard.type(deployFundsDescription);
    ...
    await page.screenshot({
      path: `test-results-production/debug-deploy-funds-after-member-${Date.now()}.png`,
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: `test-results-production/create-deploy-funds-form-${timestamp}.png`,
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: `test-results-production/create-deploy-funds-before-publish-${timestamp}.png`,
      fullPage: true,
    });
    ...
    console.log(
      '⏳ Waiting for deploy funds creation (this may take up to 2 minutes)...',
    );
    ...
    await page.screenshot({
      path: `test-results-production/deploy-funds-after-loading-${timestamp}.png`,
      fullPage: true,
    });
    ...
    try {
      ...
      await page.screenshot({
        path: `test-results-production/create-deploy-funds-error-${timestamp}.png`,
        fullPage: true,
      });
      ...
      if (currentUrl.includes('/create')) {
        ...
        if (realErrors.length > 0) {
          throw new Error(
            `Deploy funds creation failed with error: ${realErrors.join(', ')}`,
          );
        }
        throw new Error(
          'Deploy funds creation did not complete - still on create page',
        );
      }
      throw new Error('Deploy funds creation did not complete in expected time');
    }

    console.log('✅ Deploy funds creation appears to have succeeded!');
    ...
    const createdDeployFundsUrl = page.url();
    console.log(`📍 Navigated to: ${createdDeployFundsUrl}`);

    // Take a success screenshot
    await page.screenshot({
      path: `test-results-production/create-deploy-funds-success-${timestamp}.png`,
      fullPage: true,
    });
    ...
    if (await notFoundIndicator.isVisible().catch(() => false)) {
      throw new Error('Deploy funds page shows error or not found');
    }

    // Log the created deploy funds URL
    console.log('');
    console.log(
      '╔════════════════════════════════════════════════════════════════╗',
    );
    console.log(
      '║  ✅ DEPLOY FUNDS PROPOSAL CREATED SUCCESSFULLY                 ║',
    );
    console.log(
      '║                                                                ║',
    );
    console.log(`║  URL: ${createdDeployFundsUrl.substring(0, 54).padEnd(54)}║`);
    console.log(
      '║                                                                ║',
    );
    console.log(
      '║  ⚠️  Remember to reject/delete this test proposal!             ║',
    );
    console.log(
      '╚════════════════════════════════════════════════════════════════╝',
    );
    ...
    expect(createdDeployFundsUrl).not.toContain('/create');
  });
});
