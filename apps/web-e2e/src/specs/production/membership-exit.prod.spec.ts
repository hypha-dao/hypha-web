...
async function selectExitingMember(page: Page): Promise<boolean> {
  ...
  if (!memberDropdown) {
    ...
    if (
      await exitingMemberSection.isVisible({ timeout: 3000 }).catch(() => false)
    ) {
      // Look for dropdown button after this label
      memberDropdown = page
        .locator('text=Exiting Member')
        .locator(
          'xpath=following::button[contains(@class, "combobox") or @role="combobox" or contains(., "Martin")]',
        )
        .first();
    }
  }
  ...
  if (
    !memberDropdown ||
    !(await memberDropdown.isVisible({ timeout: 1000 }).catch(() => false))
  ) {
    memberDropdown = page
      .locator(
        'button:has(svg):has-text("Martin"), button[aria-haspopup="listbox"]',
      )
      .first();
  }
  ...
  if (
    memberDropdown &&
    (await memberDropdown.isVisible({ timeout: 3000 }).catch(() => false))
  ) {
    // Check if a member is already selected
    const dropdownText = (await memberDropdown.textContent()) || '';
    if (
      dropdownText.includes('Martin') ||
      dropdownText.includes('test') ||
      dropdownText.includes('Prate')
    ) {
  ...
  const memberDisplay = page
    .locator('text=Martin Prate, text=Martin test')
    .first();
...
test.describe('Membership Exit on Production', () => {
  ...
  test('should create membership exit proposal in QA TESTING space', async ({
    ...
    console.log(
      '║  🚪 CREATING MEMBERSHIP EXIT PROPOSAL ON PRODUCTION            ║',
    );
    console.log(
      '╚════════════════════════════════════════════════════════════════╝',
    );
    ...
    for (let i = 0; i < 15; i++) {
      const membershipExitVisible = await page
        .locator('text=Membership Exit')
        .first()
        .isVisible()
        .catch(() => false);
    ...
    const membershipExitCard = page
      .locator('text=Remove a member from your space if they infringe')
      .first();

    if (
      await membershipExitCard.isVisible({ timeout: 5000 }).catch(() => false)
    ) {
    ...
    console.log(
      '║  ✅ MEMBERSHIP EXIT PROPOSAL COMPLETED                         ║',
    );
    console.log(
      '║                                                                ║',
    );
    ...
    console.log(
      '║    👤 Exiting Member: Selected from dropdown                   ║',
    );
    console.log(
      '║                                                                ║',
    );
    console.log(
      '║  ⚠️  This proposal is now LIVE on the QA TESTING space!        ║',
    );
    console.log(
      '╚════════════════════════════════════════════════════════════════╝',
    );
