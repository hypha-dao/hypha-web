 ...
 */
async function fillTokenBasedFields(
  page: Page,
  tokenAmount: number,
): Promise<void> {
  ...
  if (await tokenAmountInput.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    const altInput = page
      .locator('text="Required Min. Token"')
  ...
  const tokenDropdown = page
    .locator(
      'button:has-text("USDC"), button:has-text("Select"), [role="combobox"]',
    )
    .last();
  ...
  if (await tokenDropdown.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    if (optionCount > 0) {
      ...
      if (!tokenSelected) {
        ...
        for (const token of allowedTokens) {
          ...
          if (
            await tokenOption.isVisible({ timeout: 500 }).catch(() => false)
          ) {
    ...
    if (!tokenSelected) {
      console.log(
        '⚠️ Could not find an allowed token - using keyboard selection',
      );
...
test.describe('Change Entry Method on Production', () => {
  ...
  test('should change entry method with randomized settings in the QA TESTING space', async ({
    ...
    if (needsTokenFields) {
      ...
      console.log(
        `║    🪙 Token: (will select non-USDC/ETH/BTC token)              ║`,
      );
    }
    console.log(
      '╚════════════════════════════════════════════════════════════════╝',
    ...
    for (let i = 0; i < 10; i++) {
      const entryMethodVisible = await page
        .locator('text=Entry Method')
        .first()
        .isVisible()
        .catch(() => false);
    ...
    const entryMethodCard = page
      .locator(
        'text=Select and configure the process by which new members join',
      )
      .first();
    ...
    const proposalDescription = `Automated E2E test changing entry method to "${selectedEntryMethod}"${
      needsTokenFields ? ` with ${tokenAmount} tokens required` : ''
    }.`;
    ...
    console.log(
      `🚪 Step 1: Selecting entry method "${selectedEntryMethod}"...`,
    );
    ...
    for (let i = 0; i < 5; i++) {
      const publishButton = page
        .locator(
          'button:has-text("Publish"), button:has-text("Save"), button:has-text("Apply")',
        )
        .last();
    ...
    if (needsTokenFields) {
      console.log(`║    💰 Token Amount: ${String(tokenAmount).padEnd(40)}║`);
      console.log(
        `║    🪙 Token: (selected non-USDC/ETH/BTC token)                 ║`,
      );
    }
    console.log(
      '║                                                                ║',
