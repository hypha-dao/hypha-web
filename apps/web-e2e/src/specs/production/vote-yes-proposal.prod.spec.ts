...
test.describe('Vote Yes on Proposal on Production', () => {
  ...
  test('should vote yes on the first proposal in the QA TESTING space', async ({
    ...
    const proposalCards = page
      .locator('a[href*="/agreements/"], div[class*="cursor-pointer"]')
      .filter({
        has: page.locator(
          'text=/On Voting|Voting Method|Entry Method|Issue Token|Mint|Deploy/',
        ),
      });
    ...
    const proposalTitles = page
      .locator('text=/E2E.*Change|E2E.*Method|Issue Token|Mint.*Treasury/')
      .first();
    ...
    if (await voteYesButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const alreadyVoted = page
        .locator('text=/already voted|vote recorded|voted/i')
        .first();
    ...
    try {
      await Promise.race([
        page.waitForSelector('text=/vote.*recorded|voted|success/i', {
          timeout: 5000,
        }),
      ...
      console.log(
        '⚠️ No explicit vote confirmation - vote may have been recorded',
      );
    }
    ...
    console.log(
      `║  📋 Proposal: ${proposalTitle.substring(0, 46).padEnd(46)}║`,
    );
    console.log(
      '║  🗳️ Vote: YES                                                  ║',
    );
