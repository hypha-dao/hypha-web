...
**Contract direction:** Keep **`get_documents_by_space_slug`** stable for SQL **documents**. **`get_org_memory_by_space_slug`** is the sibling read-only tool: same member roster as **`get_people_by_space_slug`**, plus **`org_memory_assets`** that can include **proposal-backed** rows today and **Matrix-backed** rows (with **`matrix_fetch`** diagnostics) when the deployment and session support it — not an always-empty array. See **`docs/requirements/mcp-get-org-memory-by-space-slug-tech-spec.md`** and **§8** in the MCP documents spec (PR #2141 context).
