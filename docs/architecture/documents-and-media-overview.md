...
**Contract direction:** Keep **`get_documents_by_space_slug`** stable for SQL **documents**. **`get_org_memory_by_space_slug`** is specified as the sibling read-only tool (**v1:** same member roster as **`get_people_by_space_slug`** + empty **`org_memory_assets[]`** until the catalogue ships). See **`docs/requirements/mcp-get-org-memory-by-space-slug-tech-spec.md`** and **§8** in the MCP documents spec (PR #2141 context).
