...
2. **Core read API** — **`getOrgMemoryBySpaceSlug`** in `@hypha-platform/core/server` returns **`members`** (via **`getSpaceMembersRoster`**) + **`org_memory_assets`** (proposal uploads from **`documents`**; Matrix **`m.file` / `m.image`** via Client-Server API when **`HYPHA_MATRIX_ORG_MEMORY_ACCESS_TOKEN`** is set) + **`assets_pagination`**. **HTTP:** `GET /api/v1/spaces/[spaceSlug]/org-memory` (same **`checkSpaceAccess`** as other space APIs; optional Bearer for MCP-style parity). When a dedicated catalogue table exists, extend this function instead of adding a parallel query shape.
