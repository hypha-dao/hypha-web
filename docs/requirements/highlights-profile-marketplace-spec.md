# Technical Specification — Highlights Profile & Marketplace Network
...
| Scope             | Space **Highlights** tab (qualitative funding profile), **Story** signal type, inline block editor, support CTAs, and **Marketplace** network view                                |
| Primary surfaces  | DHO `@tab/highlights`, Network explore (`ExploreSpaces`), Coherence signals                                                                                                       |
...
Resource providers (donors, investors, partners) need a **marketplace view** on the Network where spaces that opt in can showcase their ask. Clicking a card opens the space Highlights page with clear CTAs linked to treasury wallet or IBAN.
...
- Network list/map with discoverability filters but no **Marketplace** view for published funding asks
...
| ------------------------ | --------------------- | -------------------------------------------- |
| Space tab (first in nav) | **Highlights**        | `/{lang}/dho/{slug}/highlights`              |
| Network marketplace view | **Marketplace**       | `/{lang}/network?view=marketplace`           |
| New signal type          | **Story**             | `coherences.type = 'Story'`                  |
| Opt-in publish flag      | `highlightsPublished` | `space_highlight_profiles.published`         |
| i18n namespace           | `HighlightsTab`       | All locales in `packages/i18n/src/messages/` |

**Rationale:** _Highlights_ covers mission, needs, stories, and support CTAs without sounding purely transactional. _Marketplace_ is clear for resource providers browsing funding asks.

---

## 3) UX goals

- **Highlights tab is first** in DHO navigation — the primary public-facing story of the space.
...
- Spaces **opt in** to share their ask on the Network Marketplace view (list, map, dedicated view).
...
│  ☐ Share this ask on the Hypha Marketplace              │
└─────────────────────────────────────────────────────────┘
```
...
### 4.3 Network Marketplace view

Extend `/network` with `view=marketplace` (alongside existing `list` and `map`):

| View            | Behavior                                                        |
| --------------- | --------------------------------------------------------------- |
| List            | Existing space cards + optional highlights badge when published |
| Map             | Existing pins; popup includes highlights summary when published |
| **Marketplace** | Grid/list of **published highlights profiles only**             |

Card click → `/{lang}/dho/{slug}/highlights` (not overview).

**Marketplace card fields:**

- Cover image (`highlightsCoverImage` or `leadImage`)
...
Story signals are **narrative proof**, not workflow items. They appear on the Highlights tab and optionally in Coherence with a “Story” filter.
...
“Add story” on Highlights tab → lightweight form (title, date, photos, body) → creates coherence row with `type: 'Story'`.
...
| **`highlightsPublished`** (new)                   | Shows in Marketplace view + highlights card on list/map |
| `highlightsPublishedAt`                           | Sort “recently published”                               |
| `highlightsSummary`                               | Card teaser (max 160 chars)                             |
| `highlightsCoverImage`                            | Card image; defaults to `leadImage`                     |
| `highlightsGoalAmount` / `highlightsGoalCurrency` | Optional display-only goal                              |

### 7.2 Publish validation

`highlightsPublished === true` requires:
...
| Viewer       | Highlights tab           | Edit | Publish | See CTAs (IBAN/wallet)     |
| ------------ | ------------------------ | ---- | ------- | -------------------------- |
| Anonymous    | If `highlightsPublished` | No   | No      | If published + CTA enabled |
| Network user | If published             | No   | No      | Yes                        |
| Member       | Always                   | Yes  | Yes     | Yes                        |
| Delegate     | Always                   | Yes  | Yes     | Yes                        |

Never expose IBAN/wallet in API unless `highlightsPublished` or caller is member. Reuse `checkSpaceAccess` + transparency checks from `packages/epics/src/spaces/utils/transparency-access.ts`.

---

## 8) Data model

### 8.1 New table: `space_highlight_profiles`

```typescript
// packages/storage-postgres/src/schema/space-highlight-profile.ts
spaceHighlightProfiles = pgTable('space_highlight_profiles', {
...
```typescript
type HighlightsBlockType = 'about' | 'mission' | 'needs' | 'useOfFunds' | 'whyNow' | 'gallery' | 'location' | 'links' | 'cta' | 'stories';
...
| ------ | ------------------------------------------ | ----------------------------------- | ---------------------------------- |
| GET    | `/api/v1/spaces/{slug}/highlights`         | Public if published; members always | Full profile + Story signals       |
| PUT    | `/api/v1/spaces/{slug}/highlights`         | Member/delegate                     | Update blocks, CTAs, summary       |
| PATCH  | `/api/v1/spaces/{slug}/highlights/publish` | Member/delegate                     | Toggle `published` with validation |
| GET    | `/api/v1/network/marketplace`              | Public                              | Paginated published profiles       |

### 9.1 GET highlights response (recommended)
...
### Phase A — Highlights tab (MVP)

1. DB migration + schema export
2. Core queries/mutations in `packages/core/src/highlights/server/`
3. API routes under `apps/web/src/app/api/v1/spaces/[spaceSlug]/highlights/`
4. Route: `apps/web/src/app/[lang]/dho/[id]/@tab/highlights/page.tsx`
5. Nav: first item in `packages/epics/src/common/ai-left-panel.tsx`
6. Components in `packages/epics/src/highlights/`:
   - `HighlightsPage` — public/member view
   - `HighlightsBlockEditor` — inline `+`, reorder, gallery upload
   - `HighlightsStoriesSection` — Story coherences timeline
   - `HighlightsSupportBar` — CTA buttons
   - `HighlightsPublishPanel` — opt-in toggle + validation
7. i18n: `HighlightsTab` namespace (en, de, pt, es, fr, mk)

### Phase B — Story signal type

1. Add `Story` to `COHERENCE_SIGNAL_TYPES` + validation
2. “Add story” from Highlights tab
3. Coherence tab filter includes Stories

### Phase C — Marketplace network

1. `GET /api/v1/network/marketplace`
2. `ExploreSpaces`: add `view=marketplace`
3. `MarketplaceCard` component
4. Map popup enrichment for published spaces

### Phase D — Polish

- Share link + OG meta for highlights page
...
| #   | Ticket                   | Scope                                                |
| --- | ------------------------ | ---------------------------------------------------- |
| 1   | DB schema + CRUD API     | `space_highlight_profiles`, core server, REST routes |
| 2   | Highlights tab read-only | Route, public page, member preview                   |
| 3   | Inline block editor      | `+`, reorder, gallery upload, template               |
| 4   | Story signal type        | Coherence type, form, Highlights stories section     |
| 5   | Support CTAs             | Treasury/banking integration                         |
| 6   | Publish toggle           | Validation, permissions                              |
| 7   | Marketplace network      | List/map/view + cards + API                          |
...
10. Separate Highlights from Overview — Overview = metrics; Highlights = qualitative ask.
...
| --- | --------------------------- | ---------------------------------------------------------------------- |
| 1   | Default landing tab         | Keep overview as default URL; promote Highlights as **first nav item** |
...
| 6   | CTA default labels          | i18n keys: `HighlightsTab.actions.donate`, `.invest`, `.support`       |
