...
| **FR-OC2** | `inputSchema`: **`space_slug`**, **`page`**, **`page_size`**, optional **`searchTerm`** (roster; same constraints as people tool); optional **`assets_page`**, **`assets_page_size`**, **`assets_search`** for the **`org_memory_assets`** slice (same Zod constraints as MCP / core).                                                                                                                                                                                                                                                                      |
...
| **FR-OC9**  | When explaining **missing Matrix** rows, the model SHALL use **`matrix_fetch`** holistically: **`access_token_configured`** is **only** the bot env **`HYPHA_MATRIX_BOT_AS_TOKEN`**; **`used_session_matrix_token`** means the viewer’s Human Chat Matrix token was used; **`session_matrix_token_unavailable`** means the JWT was present but no valid Matrix link. The model SHALL **not** claim Matrix is impossible **solely** because the bot token env is unset. |
| **FR-OC10** | Register **`fetch_org_memory_asset`** in **`createChatTools`**. Tool uses **`fetchOrgMemoryAsset`** with the same auth + **`requestUrlForSessionMatrix`** as org memory listing. **`toModelOutput`** maps successful **`image/*`** results to multimodal **`image-data`** so vision-capable models **see** screenshots; text/PDF results use **`text`**. List files with **`get_org_memory_by_space_slug`** first; pass **`asset_key`** from each row.                            |
