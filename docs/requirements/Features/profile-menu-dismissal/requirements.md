...
|----|----------|-------|--------|
| OQ-1 | Which routes and layouts render the profile menu (web only vs. other apps)? | Engineering | Before implementation |
| OQ-2 | Is the profile surface a `DropdownMenu`, `Popover`, or responsive `Sheet`? | Design / Engineering | Before test locators finalized |
| OQ-3 | Are any menu items required to keep the menu open after click (e.g. “Copy address”)? | Product | Before FR-1 sign-off |
