...
1. **Coherence, Space Memory, AI Chat (left):** **Hypha cookie** if set to `"true"` or `"false"`; else **`NEXT_PUBLIC_*`** if set; else default **`true`**.
2. **Human Chat (right):** kill-switch first; else cookie / env; else default **`false`**. Set **`HYPHA_ENABLE_HUMAN_CHAT=true`** to opt in.
