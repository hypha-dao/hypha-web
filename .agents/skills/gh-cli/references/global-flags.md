...
> Note: Flags like `--cache`, `--hostname`, and `--paginate` are primarily for `gh api`. Others like `--json`, `--jq`, and `--template` are output formatting options not available on all commands. Check `gh COMMAND --help` for command-specific flags.
